...
class SerialLoraBridge:
    ...
    def start(self):
        ...
        self.log("Type 'poll <DevAddr> <NwkSKey> <AppSKey>' to send Health_Req", C_YELLOW)
    ...
    def send_downlink_command(self, dev_addr_hex, nwk_skey_hex, app_skey_hex, opcode1, opcode2, fcnt_down=0):
        """
        Downlink Data 封包
        ...
        try:
            ...
            #       2: (0x03) 要求回覆3號指令
            if opcode1 == 0x40:
                 frm_payload_plain = bytes([opcode1, opcode2, 0x01, 0x03])  # OpCode + Length=1 + 要求回覆3號指令 0x03
    ...
    def input_loop(self):
		...
		#// w: =====
                    ...
                    if len(parts) < 4:
                        ...
                        self.send_downlink_command(dev_addr, nwk_skey, app_skey, 0x10, 0x11, fcnt)  # 0x01還是0x11??

                elif op == "amp":
                    # cmd: amp <DevAddr> <NwkSKey> <AppSKey> [FCntDown]
                    # exp: amp 01A2B3C4 AABBCCDD...16bytes... 11223344...16bytes... 0
                    if len(parts) < 4:
                        self.log("Usage: amp <DevAddr> <NwkSKey> <AppSKey> [FCntDown]", C_RED)
                        self.log("  DevAddr: 4 bytes hex (e.g., 01A2B3C4)", C_YELLOW)
                        self.log("  NwkSKey: 16 bytes hex (32 chars)", C_YELLOW)
                        self.log("  AppSKey: 16 bytes hex (32 chars)", C_YELLOW)
                        self.log("  FCntDown: optional, default=0", C_YELLOW)
                        ...
                        self.send_downlink_command(dev_addr, nwk_skey, app_skey, 0x40, 0x01, fcnt)  # (0x03) 要求回覆3號指令
                    ...
                    if len(parts) < 4:
                        ...
                        app_skey = parts[3]
                        fcnt = int(parts[4]) if len(parts) > 4 else 0
                        self.send_downlink_command(dev_addr, nwk_skey, app_skey, 0x20, 0x01, fcnt)
